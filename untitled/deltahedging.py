...
vega = S0*stats.norm.cdf(n1,0.0,1.0)*np.sqrt(T)
...
def greeksdata(S0,r,sigma,T,M,I,K):
    ...
    for t in range(1,M+1):
        rand = np.random.standard_normal(I)
        greeks[t][0] = greeks[t - 1][0] * np.exp((r - 0.5 * sigma ** 2) * dt + sigma * np.sqrt(dt) )
        d1 = (np.log(greeks[t][0] / K) + (r + 0.5 * sigma ** 2) * T) / (sigma * np.sqrt(T))
        d2 = (np.log(greeks[t][0] / K) - (r + 0.5 * sigma ** 2) * T) / (sigma * np.sqrt(T))

        greeks[t][1] = stats.norm.cdf(d1,0.0,1.0)
        greeks[t][2] = stats.norm.cdf(d1,0.0,1.0)/(greeks[t][0]*T**0.5)
        greeks[t][3] = (greeks[t][0]*stats.norm.pdf(d1,0.0,1.0)*sigma)/(2*np.sqrt(T))\
                       -r*K*np.exp(-r*T)*stats.norm.cdf(-d1,0.0,1.0)
        greeks[t][4] = K*T*np.exp(-r*T)*stats.norm.cdf(d2,0.0,1.0)
        greeks[t][5] = greeks[t][0]*stats.norm.cdf(d1,0.0,1.0)*np.sqrt(T)
...
print('Your finall call payoff with GBM is',CallPayoff, '. The final delta value is ')
